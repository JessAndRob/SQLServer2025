-- =====================================================================
-- Parameters — override from setup\05-deploy-customer-feedback-data.ps1
...
-- =====================================================================
-- demo\03-summarise-and-inject.ps1 walks an audience through the procs
-- in setup\06-chat-procedures.sql. This file owns the schema and the
-- *clean* seed data — every row below is a normal piece of feedback.
--
-- The injection row (Justin Grote's "fake system update" attack — Justin
-- is happy to be made fun of) is NOT in the seed. It is added at demo
-- runtime by the PowerShell runner so the defended proc can be shown
-- working perfectly first, before the vulnerable proc is hit with the
-- attack.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- Customers — 26 PSConfEU organisers and speakers.
-- The CreditCard column is intentionally awful; it's the dramatic-effect
-- column for the "look what your AI assistant just exfiltrated" reveal.
-- These are public test card numbers (Stripe / scheme test ranges), not
-- real card data.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Feedback — keyed on CustomerId (FK), ON DELETE CASCADE so removing a
-- customer (e.g. cleaning Mallory up after the demo) takes their
-- comments with them.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 100 clean feedback entries — generally supportive in tone with a
-- recurring complaint about documentation. The (VALUES ...) shape lets
-- you read (name, comment) together; the JOIN turns the name into the
-- right CustomerId.
-- ---------------------------------------------------------------------
...
FROM (VALUES
 ...
 -- Two rows from Gael Colas so he shows up alongside the usual suspects
 -- when the demo pulls a sample of feedback.
